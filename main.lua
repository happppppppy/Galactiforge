...
--Object components
require("code/src/components/objects/Ship")
require("code/src/components/objects/GridShip")
local Ship, GridShip  = Component.load({"Ship", "GridShip"})
...
--Controller systems
GridShipSystem = require("code/src/systems/controllers/GridShipSystem")
...
RenderGridShipSystem = require("code/src/systems/graphic/RenderGridShipSystem")
...
function love.load()
	...
	eventmanager:addListener("GridSelected", GridShipSystem(), GridShipSystem().fireEvent)
	...
	engine:addSystem(RenderGridShipSystem())
	engine:addSystem(GridShipSystem())
	...
	playerShip = Entity()
	playerShip:add(Ship("intruder"))
	engine:addEntity(playerShip)
	ship_vals = playerShip:get("Ship")
	...
	playerShip:add(DynamicPhysics(10000, 250000, 200, 200, 0.2, 0.2))
	playerShip:add(CollisionPhysics("Rectangle",  ship_vals.grid_size.width*ship_vals.grid_scale, ship_vals.grid_size.height*ship_vals.grid_scale, 1, 1, 1))
	playerShip:add(PlayerController())
	playerShip:add(GridShip(grid))
	playerShip:add(Health(100))
	playerShip:add(Faction("Terran"))

	opponentShip = Entity()
	opponentShip:add(Ship("intruder"))
	...
	opponentShip:add(DynamicPhysics(50, 100, 200, 200, 0.8, 0.7))
	opponentShip:add(CollisionPhysics("Rectangle",  20, 50, 1, 2, 2))
	opponentShip:add(AIController())
	opponentShip:add(GridShip(grid))
	opponentShip:add(Health(100))
	opponentShip:add(FieryDeath())
	opponentShip:add(Faction("Splorg"))
	engine:addEntity(opponentShip)

	destructable = Entity()
	destructable:add(Health(10))
	destructable:add(Ship("station"))
	destructable:add(GridShip(station_grid))
