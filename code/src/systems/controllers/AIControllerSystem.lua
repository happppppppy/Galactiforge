  ...
  if global_ai_active then
    for index, value in pairs(self.targets) do
      ...
      for _,target in pairs(master_grids) do
        local target_faction = target:get("Faction")
        if target_faction.faction ~= value:get("Faction").faction then
          local target_physics = target:get("PositionPhysics")
            ...
            for _,weapon in pairs(value.children) do
              ...
              if weapon_component ~= nil then
                ...
                for _,target_grid in pairs(target.children) do
                  local target_grid_item = target_grid:get("GridItem")
      ...
      if AIController.target_direction ~= AIPhysics.body:getAngle() and AIController.target_range < AIController.agro_range then
      ...
      if AIController.target_range > AIController.range and AIController.target_range < AIController.agro_range then
